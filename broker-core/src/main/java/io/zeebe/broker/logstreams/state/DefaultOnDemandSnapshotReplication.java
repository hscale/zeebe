...
public class DefaultOnDemandSnapshotReplication
  ...
  public DefaultOnDemandSnapshotReplication(
      ClusterCommunicationService communicationService, int partitionId, String name) {
  ...
  public void request(MemberId server) {
    communicationService.unicast(requestReplicationTopic, true, server);
  }

  @Override
  public void serve(Handler handler) {
    executorService =
        Executors.newSingleThreadExecutor((r) -> new Thread(r, requestReplicationTopic));
    communicationService.subscribe(
        requestReplicationTopic,
        (Consumer<Boolean>) r -> handler.onSnapshotRequest(r, executorService),
        executorService);
  }
